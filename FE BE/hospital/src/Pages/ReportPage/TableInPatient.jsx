...
import { baseUrl, urls } from '../../router/axios_api.js';
...
export default function TableInPatient({showTreatment, setShowTreatment, setRecordTreatmentShow}) {
  ...
  useEffect(() => {
    async function getData() {
      let data = await baseUrl.get(urls.ipPatient)
        ...
        <TableBody>
          ...
          .map((row) => (
            ...
            >
              ...
              <TableCell align="right">{row.sick_room}</TableCell>
              <TableCell align="right">{row.nurse.map(n => <p>{n}</p>)}</TableCell>
              <TableCell align="right">{row.total_fee}</TableCell>

...
export default function TableContent() {
  ...
  useEffect(() => {
    async function getData() {
      let data = await baseUrl.get(urls.searchPatient)
      if(data.data)
        ...
        <TableBody>
          ...
          .map((row) => (
            ...
            >
              ...
              <TableCell align="right">{row.doctor.map((dc,i) => <p key ={i}>{dc}</p>)}</TableCell>
              <TableCell align="center" sx={{width: 24, height: 24}}>
              <Link to="/Home/Report" onClick={() => onClickFunc(row._id)}> 
                <ArrowRightIcon/>
              </Link>
              </TableCell>
            </TableRow>
